...
def post_listing_to_slack(sc, listing):
    ...
    """
    desc = "{0} | {1} | {2} | {3} | <{4}>".format(listing["price"], listing["bart"], round(listing["bart_dist"], 2) if type(listing["bart_dist"]) == float else listing["bart_dist"], listing["name"], listing["url"])
    sc.api_call(
...
def find_points_of_interest(geotag, location):
    ...
    for station, coords in settings.TRANSIT_STATIONS.items():
        ...
        if (bart_dist is None or dist < bart_dist) and dist < settings.MAX_TRANSIT_DIST:
            bart = station
            near_bart = True
            bart_dist = dist
